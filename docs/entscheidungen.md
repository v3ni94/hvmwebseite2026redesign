...
| 24.09.2026 | `config/staedte.php` ersetzt `config/standorte.php`, Stadtseiten `/hausverwaltung-<slug>/` | 42 PLZ-Bereiche laut Geschäftsführung. Kein Status eigene Präsenz oder Partner mehr, keine Anschriften je Stadt (offen, ob die Absenderadressen eigene Büros sind). Stadttexte als Markdown in `content/staedte/` mit Freigabelogik wie Wissensartikel; Indexierung nur bei lokalem Bezug (`indexierbar`), siehe `docs/seo-geo.md` Abschnitt 8. Kartenposition wird aus `geo` berechnet (`Stadt::karte()`, gleiche Projektion wie der Umriss), statt vorberechnet gepflegt. |
| 24.09.2026 | Platzhalter-Scanner ignoriert Markdown-Links | `[Linktext](/pfad/)` ist kein Platzhalter, auch wenn der Linktext „prüfen“ o. ä. enthält (Fehlalarme aus Stadttexten und Artikeln). |
| 24.09.2026 | Zweiter Betriebsweg: Webhosting per SFTP (Apache, PHP-FPM, ohne Kommandozeile) | Staging `neu.muellerhv.de` hat nur SFTP-Zugang. `public/.htaccess` und eine Wurzel-`.htaccess` (Document Root nicht änderbar) ersetzen die Nginx-Konfiguration, alle Module mit `IfModule` abgesichert. Release-Paket aus `bin/build-release.sh` mit `vendor`, Build und `.env` mit frischen Schlüsseln, nie im Repository. Anleitung `docs/deploy-sftp.md`. |
| 24.09.2026 | Web-Einrichtung `/_einrichtung/` statt Kommandozeile | Nur mit `SETUP_TOKEN` (mindestens 32 Zeichen) und ohne `storage/setup.lock`, sonst 404 in jeder Umgebung. Token per Formular (`hash_equals`), Rate Limit über Dateien (funktioniert vor den Migrationen), CSRF, Sitzung an den Token gebunden. Diagnose (`Hvm\Support\Diagnose`, auch von `bin/diagnose.php` genutzt) und Migrationen (`Hvm\Support\Migrator`, auch von `bin/migrate.php` genutzt) sind gemeinsamer Code. Erster Admin über `Hvm\Security\AdminProvisioning` (auch `bin/admin-user.php create`), Geheimnis und Codes nur einmal in der Antwort. |
| 24.09.2026 | `OUTBOX_MODE=inline` für Hosting ohne Worker und Cronjob | Nach der Antwort (`Kernel::terminate`, `fastcgi_finish_request`) höchstens 5 fällige Einträge; nach POST sofort, bei GET höchstens alle 5 Minuten (Wiederholungen). Dateisperre plus Status-Claim der Outbox gegen Doppelversand. Standard bleibt `worker`. |
| 24.09.2026 | Staging-Schutz auch in der Anwendung (`StagingBasicAuth`) | Ohne Traefik gibt es keine vorgeschaltete Basic Auth. Aktiv nur bei `APP_ENV=staging`, bcrypt- oder Argon2-Hash, `/health` frei. apr1-Hashes (Traefik) werden ignoriert, damit Docker-Staging nicht doppelt fragt; jeder andere nicht erkennbare Wert sperrt. |
