# Angaben des Auftraggebers

Verbindliche Angaben der Geschäftsführung, die Platzhalter aus dem Masterprompt auflösen. Nur hier dokumentierte Angaben dürfen als bestätigt verwendet werden.

| Datum | Thema | Angabe | Umsetzung |
|---|---|---|---|
| 23.09.2026 | Datenleck Altseite (Phase 0) | In Arbeit, noch nicht abgeschlossen | docs/phase0.md |
| 23.09.2026 | Zentrale Telefonnummer | 02431 9550300 gilt weiterhin (international +49 2431 9550300) | config/unternehmen.php |
| 23.09.2026 | 24/7-Notdienst | zutreffend, darf genannt werden. Notfallnummer selbst noch offen | Texte, Notfallseite |
| 23.09.2026 | Eigentümerportal mit Ticketsystem | zutreffend, darf genannt werden. Portal-URL noch offen | Texte, Serviceseite |
| 23.09.2026 | Rahmenverträge (Versicherung, Gas, Strom, Hausmeister, Messdienst) | zutreffend, darf genannt werden | Texte |
| 23.09.2026 | Keine Aufnahmegebühr | zutreffend, darf genannt werden | Texte, FAQ Kosten und Vertrag |
| 23.09.2026 | Hauseigene Techniker | nicht bestätigt, bleibt als [bestätigen] markiert | Texte |
| 23.09.2026 | Freigabe-Rhythmus | Durchbauen bis staging-fähig, Freigaben gesammelt danach über config/freigaben.php | Projektablauf |
| 23.09.2026 | Asset Management als eigene Leistung | bestätigt („ja, setze um“) | Seite /asset-management/, Artikel |
| 23.09.2026 | Leistungsumfang Miet- und SE-Verwaltung: Mieterhöhungen, Indexanpassungen, Kautionsverwaltung, Wohnungsübergaben, Begleitung von Modernisierungen, CO2-Kostenaufteilung | bestätigt | Leistungsseiten, Artikel |
| 23.09.2026 | Verkaufsbegleitung: Marktwerteinschätzung, Unterlagen zusammenstellen, Energieausweis prüfen, Begleitung bis zum Notartermin (Beurkundung durch den Notar, keine Aussage zur Provision) | bestätigt | Seite Verkauf, Artikel |
| 23.09.2026 | Wertermittlung: kaufmännische Marktwerteinschätzung durch die HVM, Gutachten durch qualifizierte Sachverständige | bestätigt. Namen und Qualifikation der Sachverständigen weiterhin offen | Seite Wertgutachten, Artikel |
| 23.09.2026 | Energieausweise stellt die HVM nicht selbst aus | bestätigt | Artikel Energieausweis |
| 23.09.2026 | Online-Teilnahme an Eigentümerversammlungen bzw. virtuelle Versammlungen werden angeboten | bestätigt | Artikel online-eigentuemerversammlung |
| 23.09.2026 | Leistungsverzeichnis Grund- und Sonderleistungen existiert | bestätigt. Inhalt bzw. Dokument noch nicht übergeben, Verweis bleibt Platzhalter | Artikel sonderleistungen-verwaltung |
| 23.09.2026 | Entwurf des Verwaltervertrags wird mit dem Angebot übersandt | bestätigt | Artikel verwaltervertrag-inhalte |
| 24.09.2026 | Notfallnummer | 02431 9550300 (zentrale Nummer), außerhalb der Sprechzeiten in der Leitung bleiben | config/unternehmen.php, Notfallseite |
| 24.09.2026 | Portal-URL | portal.muellerhv.de | config/unternehmen.php |
| 24.09.2026 | Bürozeiten | 8 bis 16 Uhr (Wochentage nicht genannt, offen) | config/unternehmen.php |
| 24.09.2026 | Betreuungsgebiete | bundesweit nach 42 PLZ-Bereichen laut Absenderadressen-Tabelle der Verwaltungssoftware; je Stadt eine eigene Landingpage mit eigenständigem Inhalt gewünscht. Ob die Absenderadressen eigene Büros, Partnerbüros oder Postanschriften sind, ist offen, sie werden daher nicht als Standort veröffentlicht | config/staedte.php, Stadtseiten |
| 24.09.2026 | Wertgutachten | Erstellung durch eigens ausgebildetes Personal, teilweise durch externe Sachverständige | Seite Wertgutachten, Artikel |
| 24.09.2026 | Logo-Nachzeichnung (SVG) | freigegeben | public/assets/img/logo, docs/logo |
| 24.09.2026 | Impressum und Datenschutz | Rückmeldung Anwalt: „sieht sehr gut aus“. Inhaltliche Platzhalter (u. a. Erlaubnis § 34c GewO und Behörde, Berufshaftpflicht, Verbraucherschlichtung, Datenschutzbeauftragter, AVV) sind noch zu füllen | Pflichtseiten |
| 24.09.2026 | Verwalterwechsel: HVM übernimmt nach Beschluss die Abstimmung zur Übergabe mit dem bisherigen Verwalter, fehlende Unterlagen werden festgehalten und nachgefordert; Bürobesuche nur nach vorheriger Terminabstimmung | bestätigt („ja ist korrekt“) | Verwalterwechsel, Start, Kontakt |
| 24.09.2026 | Staging-Subdomain | neu.muellerhv.de | .env.example, docs/betrieb.md |
| 24.09.2026 | Bürozeiten, Wochentage | Montag bis Freitag, 8 bis 16 Uhr (nachgereicht über die Projektkoordination) | config/unternehmen.php, Schema openingHoursSpecification |
| 24.09.2026 | Indexierung der Stadtseiten | indexierbar nur Hauptsitz Monheim am Rhein und Städte mit bereits verwalteten Objekten (Berlin, Köln, Erkelenz, Essen, Aachen, Ulm); übrige Stadtseiten noindex, follow, nicht in Sitemap und llms.txt; freischalten bei echtem lokalem Bezug. Jede Stadtseite nennt die Betreuung durch die HVM mit Sitz in Monheim am Rhein, Termine vor Ort nach Absprache (über die Projektkoordination) | config/staedte.php, docs/seo-geo.md |
| 24.09.2026 | Leistungsverzeichnis Grund- und Sonderleistungen | aus den Muster-Angeboten (WEG, Miet, SE, Stand 24.09.2026) übernommen, ohne Preise, Namen und Objektdaten. Formulierung auf der Website allgemein („in der Regel“, individueller Vertrag maßgeblich) | docs/leistungsverzeichnis.md, Leistungsseiten, Artikel sonderleistungen-verwaltung, leistungsumfang-verwaltervertrag, verwaltervertrag-inhalte, vertragslaufzeit-verwaltervertrag |
| 24.09.2026 | Mieterhöhungen in der SE-Verwaltung | meist Sonderleistung nach Aufwand (nicht in der Grundvergütung), korrigiert die Angabe vom 23.09.2026 | SE-Seite, Artikel |
| 24.09.2026 | Arbeitsweisen: getrennte Konten der Gemeinschaft, Vorbesichtigung vor dem Auszug, datensparsame Mieterauswahl, Vergleichsangebote für Versicherungen, Abstimmung mit dem Beirat vorab, Schadensabwicklung mit dem Versicherer | bestätigt | Stadtseiten, Artikel |
| 24.09.2026 | Standorte | Büro ausschließlich in Monheim am Rhein. Die übrigen Städte sind Betreuungsgebiete mit Mitarbeitern vor Ort (keine Büros). Keine Anschriften je Stadt veröffentlichen | Stadtseiten, strukturierte Daten |
| 24.09.2026 | Speicherung der Leads | Keine eigene Datenbank der Webseite. Leads gehen direkt an den bestehenden n8n-Prozess, der sie in die bestehende Unternehmensdatenbank schreibt. Kein Admin-Bereich auf der Webseite nötig. Bisheriger Datenbankmodus bleibt als Option | `STORAGE_MODE=datei` (Standard, Staging-Paket), `docs/n8n-webhook.md`, `docs/deploy-sftp.md`, Datenschutzerklärung |
