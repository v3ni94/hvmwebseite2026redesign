...
| A4 | Aufbewahrungsfrist für Anfragen ohne Vertragsschluss festlegen (`LEAD_RETENTION_DAYS`). Im Dateimodus betrifft die Frist auf der Webseite nur fehlgeschlagene Outbox-Aufträge (technisch höchstens 30 Tage, `bin/retention.php` und stündlich inline); die Frist für Anfragen in der Unternehmensdatenbank ist dort umzusetzen | `config/app.php:57`, `docs/architektur.md` Abschnitt 5, `docs/recht-offene-fragen.md` D13 | ja |
...
| A6 | erledigt (24.09.2026): Betreuungsgebiete bundesweit nach 42 PLZ-Bereichen bestätigt, umgesetzt in `config/staedte.php` und den Stadtseiten `/hausverwaltung-<stadt>/` (ersetzt `config/standorte.php`). Standorte geklärt: Büro ausschließlich in Monheim am Rhein, die übrigen 41 Städte sind Betreuungsgebiete mit Mitarbeitern vor Ort, keine Büros und keine Anschriften je Stadt, kein LocalBusiness je Stadt. Alle 42 Stadtseiten indexierbar (`docs/seo-geo.md` Abschnitt 8). Verbleibend: Stadttexte (`content/staedte/*.md`) einzeln freigeben (`freigabe: ja`) | `config/staedte.php`, `content/staedte/`, `templates/pages/stadt.html.twig`, `docs/auftraggeber-angaben.md` | ja (je Stadtseite, Seite ohne Freigabe liefert in Produktion 404) |
| A7 | Stellenangebote für die Karriereseite ergänzen | `config/stellen.php:7` | nein |
| A8 | Teilweise erledigt (24.09.2026): Objektbegehungen laut Muster-Angeboten bedarfsgerecht (WEG) bzw. jährliche Wohnungsbegehungen (Miet, SE), `docs/leistungsverzeichnis.md`. Feste Ansprechperson je Objekt weiterhin offen | Bericht „content“ dieser Sitzung, MP Abschnitt 6/7 | nein |
...
| A21 | erledigt (24.09.2026): Vertragslaufzeiten und Kündigungsfristen aus den Muster-Angeboten übernommen (`docs/leistungsverzeichnis.md`) | `content/wissen/vertragslaufzeit-verwaltervertrag.md` | nein |
...
| A39 | erledigt (24.09.2026): Entscheidung „keine eigene Datenbank der Webseite, Leads über den bestehenden n8n-Prozess in die Unternehmensdatenbank, kein Admin-Bereich“, umgesetzt als `STORAGE_MODE=datei` (Standard, Staging-Paket). Offen: Produktions-URL des n8n-Webhooks (https) mitteilen und in `N8N_WEBHOOK_URL` eintragen, das vom Release erzeugte `N8N_WEBHOOK_SECRET` in n8n hinterlegen, Signaturprüfung und Antwortmodus (Fehler mit Status ungleich 2xx) einrichten | `docs/n8n-webhook.md`, `docs/deploy-sftp.md` Abschnitt 4, `docs/auftraggeber-angaben.md` | ja |
| A40 | Zieltabelle und Feldnamen der Unternehmensdatenbank für Angebots-, Kontakt- und Bewerbungsdaten bestätigen, `uuid` als eindeutigen Schlüssel anlegen (Idempotenz); Zuordnung zu den Altfeldern „Properties“ prüfen, Preisstufen entfallen im Dateimodus | `docs/n8n-webhook.md` Abschnitte 6 und 7 | ja |
| A41 | Empfänger der Bewerbungen festlegen (`BEWERBUNG_NOTIFY_TO`, sonst `LEAD_NOTIFY_TO`); die interne Mail enthält im Dateimodus Kontaktangaben und das PDF, Postfach entsprechend schützen | `config/app.php`, `.env.example` | nein |
| A42 | Aufbewahrung der Ausführungsdaten in n8n festlegen (enthalten personenbezogene Daten der Anfragen) | `docs/n8n-webhook.md` Abschnitt 6 | ja |
| A43 | Docker-Betrieb (`docker-compose.yml` mit MariaDB, Worker, Backup) ist auf den Datenbankmodus ausgelegt; bei Umzug auf den eigenen Server entscheiden, ob dort ebenfalls `STORAGE_MODE=datei` gilt (dann Dienste `db` und `backup` entbehrlich) oder `STORAGE_MODE=datenbank` gesetzt wird | `docker-compose.yml`, `docs/betrieb.md` | nein |
...
| B15 | Kontaktformular (Welle 2a): Felder mit Datenschutzerklärung abgleichen, Übermittlung an n8n klären. Stand 24.09.2026: alle drei Formulare werden an n8n übermittelt, Datenschutzerklärung (Abschnitt „Übermittlung und Zwischenspeicherung“) entsprechend angepasst, Platzhalter „Betrieb n8n und Speicherort der Unternehmensdatenbank klären“ und „Aufbewahrungsfrist für fehlgeschlagene Übermittlungen“ offen | `docs/recht-offene-fragen.md` D8 | ja |
...
| C21 | Staging-Abnahme im Dateimodus: Webhook kommt signiert in n8n an, Datensatz in der Unternehmensdatenbank, Mails bei `LEAD_NOTIFY_TO` und beim Absender, `storage/outbox` danach leer; lokal mit Fake-Empfängern geprüft (`tests/e2e/dateimodus.spec.js`) | `docs/deploy-sftp.md` Abschnitt 9 | ja |
| C22 | Vor Livegang prüfen, ob das Webhosting `fastcgi_finish_request` bereitstellt (sonst wartet der Besucher auf Webhook und SMTP, höchstens etwa 5 Sekunden je Webhook); andernfalls Cronjob mit `bin/worker.php --once` und `OUTBOX_MODE=worker` | `src/Http/Kernel.php` (`terminate`), `docs/deploy-sftp.md` Abschnitt 8 | nein |
...
| D4 | erledigt (24.09.2026): Mieterhöhungen in der SE-Verwaltung als Sonderleistung in der Regel nach Aufwand dargestellt (korrigiert die Angabe vom 23.09.2026), Mietverwaltung unverändert | `templates/pages/se-verwaltung.html.twig`, Artikel `sondereigentumsverwaltung`, `mietverwaltung-oder-se-verwaltung`, `mieterhoehung-vergleichsmiete`, `docs/leistungsverzeichnis.md`, `docs/auftraggeber-angaben.md` | nein |
| D5 | erledigt (24.09.2026): sechs Arbeitsweisen bestätigt (getrennte Konten der Gemeinschaft, Vorbesichtigung vor dem Auszug, datensparsame Mieterauswahl, Vergleichsangebote für Versicherungen, Abstimmung mit dem Beirat vorab, Schadensabwicklung mit dem Versicherer) | Stadtseiten, Artikel, `docs/auftraggeber-angaben.md` | nein |
